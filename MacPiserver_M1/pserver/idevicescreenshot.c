...
char getXCUIScreenShot(unsigned char** imgData, uint64_t* imageSize, char* xPort);
...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		if(getXCUIScreenShot(&imgdata, &imgsize, xPort) == SCREENSHOTR_E_SUCCESS){
			FILE *f = fopen(filename, "wb");
                        if (f) {
					if (fwrite(imgdata, 1, (size_t)imgsize, f) == (size_t)imgsize) {
                                        printf("Screenshot saved to %s\n", filename);
			...
			printf("XCUITest Could not get screenshot!\n");
		}			
	}
	idevice_free(device);
	free(filename);
