...
{
// NSString *bundleId = (id)request.arguments[@"bundleId"] ;
  ;
  NSURL *url = request.URL;
  NSLog(@"Bundle Id --------- %@--------",url.query);
  FBApplication *application;
  if(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"13.4")){
    application = request.session.activeApplication ?: [FBApplication fb_activeApplicationWithDefaultBundleId:url.query];
  }
  else
  {
