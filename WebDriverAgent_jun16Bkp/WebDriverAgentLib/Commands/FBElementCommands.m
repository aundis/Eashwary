...
+ (NSArray *)routes
{
  ...
  @[
    ...
    [[FBRoute POST:@"/wda/element/:uuid/pinchin"] respondWithTarget:self action:@selector(handlePinchin:)],
    [[FBRoute POST:@"/wda/element/:uuid/pinchout"] respondWithTarget:self action:@selector(handlePinchout:)],
    ...
    [[FBRoute POST:@"/wda/element/:uuid/dragfromtoforduration"] respondWithTarget:self action:@selector(handleDragithRapidResp:)],
    [[FBRoute POST:@"/wda/dragfromtoforduration"] respondWithTarget:self action:@selector(handleDragCoordinateWithRapidResp:)],
    [[FBRoute POST:@"/wda/tap/:uuid"] respondWithTarget:self action:@selector(handleTapCoordinateWithRapidResp:)],
...
//  XCUICoordinate *pressCoordinate;
///*
//  if(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"13.4")){
//    NSLog(@"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&Long press &&&&&&&&&&&&&&&&&&\n");
//    pressCoordinate = [self.class gestureCoordinateWithCoordinate:touchPoint application:@"com.apple.springboard" shouldApplyOrientationWorkaround:isSDKVersionLessThan(@"11.0")];
//  }
//  else { */
//    pressCoordinate = [self.class gestureCoordinateWithCoordinate:touchPoint application:request.session.activeApplication shouldApplyOrientationWorkaround:isSDKVersionLessThan(@"11.0")];
//  //}
...
//  return FBResponseWithOK();
  ...
  [[XCEventGenerator sharedGenerator] pressAtPoint:tapPoint forDuration:duration orientation:UIOrientation handler:^(XCSynthesizedEventRecord *record, NSError *error) {} ];
  return FBResponseWithOK();
}
...
+ (id<FBResponsePayload>)handleDragCoordinateWithRapidResp:(FBRouteRequest *)request
{
 ...
 /* if(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"10.0")){
  ...
  [[XCEventGenerator sharedGenerator] pressAtPoint:startPoint forDuration:duration liftAtPoint:endPoint velocity:500 orientation:UIOrientation name:@"drag" handler:^(XCSynthesizedEventRecord *record,NSError *error){}];
  return FBResponseWithOK();
}
...
+ (id<FBResponsePayload>)handlePinch:(FBRouteRequest *)request
{
  ...
  NSLog(@"Pinch  API: %f",[request.arguments[@"x"] doubleValue] );
  NSLog(@"Pinch  API: %f",[request.arguments[@"y"] doubleValue] );
  NSLog(@"Pinch  API: %f",[request.arguments[@"width"] doubleValue] );
  NSLog(@"Pinch  API: %f",[request.arguments[@"height"] doubleValue] );
  
  dispatch_semaphore_t t = dispatch_semaphore_create(0);
  
  [[XCEventGenerator sharedGenerator] pinchInRect:rect withScale:[request.arguments[@"scale"] doubleValue] velocity:1.1 orientation:UIInterfaceOrientationPortrait handler:^(XCSynthesizedEventRecord *record, NSError *error) {
    dispatch_semaphore_signal(t);
  }];
  dispatch_semaphore_wait(t, DISPATCH_TIME_FOREVER);
  NSLog(@"Pinch  API Post ");
  return FBResponseWithOK();
}

+ (id<FBResponsePayload>)handlePinchin:(FBRouteRequest *)request
{
...
+ (id<FBResponsePayload>)handlePinchout:(FBRouteRequest *)request
{
  NSLog(@"Pinch  API Pre ");
...
+ (id<FBResponsePayload>)handleGetWindowSize:(FBRouteRequest *)request
{
  ...
  NSLog(@"Before getting active application session");
  NSInteger Orientation = request.session.activeApplication.interfaceOrientation;
  

  CGSize screenSize = FBAdjustDimensionsForApplication(frame.size, Orientation);
